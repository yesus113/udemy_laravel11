...
@section('content')
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mx-4">
        ...
        <x-gauge.dht11-temperatura />
        <x-gauge.dht11-humedad />

        <div class="max-w-sm  border-gray-200 rounded-lg shadow-md bg-white  dark:border-gray-700">
            <a href="{{ route('dht11.index') }}">
                <br>
                <p class="text-black text-center font-bold">TABLE</P>


                <img src="{{ asset('static/img/table.png') }}" alt="Sensor DHT11" class="mx-auto w-48 h-48 object-contain">
            </a>
        </div>
    </div>
...
@push('scripts')
    <script>
        ...
        const fetchInitialData = async () => {
            const res = await fetch("{{ url('/dht11/last20') }}");
        ...
        const onChartLoad = function() {
            ...
            setInterval(async function() {
                const res = await fetch("{{ url('/dht11/latest') }}");
        ...
        fetchInitialData().then(data => {
            ...
            const seriesDefinitions = [{
                    name: 'humedad',
                    color: seriesColors['humedad'],
                    data: []
                },
                {
                    name: 'temperatura',
                    color: seriesColors['temperatura'],
                    data: []
                },
            ...
            data.forEach(point => {
                ...
                seriesDefinitions[1].data.push([point.x, point.y1]);
            });
