...
@section('content')
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mx-4">
        ...
        <div class="max-w-sm  border-gray-200 rounded-lg shadow-md bg-white  dark:border-gray-700">
            <a href="{{ route('lm35.index') }}">
                <br>
                <p class="text-black text-center font-bold">TABLA</P>


                <img src="{{ asset('static/img/table.png') }}" alt="Sensor LM35" class="mx-auto w-48 h-48 object-contain">
            </a>
        </div>
    </div>
...
@push('scripts')
    <script>
        ...
        const fetchInitialData = async () => {
            const res = await fetch("{{ url('/lm35/last20') }}");
        ...
        const onChartLoad = function() {
            ...
            setInterval(async function() {
                const res = await fetch("{{ url('/lm35/latest') }}");
