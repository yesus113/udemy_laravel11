...
@section('content')
    <div class="max-w-7xl mx-auto 2 sm:px-6 lg:px-4">
        <h1 class="text-2xl font-bold text-white mb-6">SENSOR TCS3200</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">R</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">G</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">B</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">FECHA</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">CONFIGURATION</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">RIESGO</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">SENSIBILIDAD UV</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">TIPO DE PIEL</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @foreach ($tcs as $t)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $t->col_R }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $t->col_G }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $t->col_B }}</td>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $t->tipo_piel->tip_tipo }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="mt-6">
            {{ $tcs->appends(request()->query())->links() }}
        </div>
    </div>
@endsection
