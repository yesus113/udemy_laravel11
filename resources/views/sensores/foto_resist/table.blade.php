...
@section('content')
    <div class="max-w-7xl mx-auto 2 sm:px-6 lg:px-4">
        <h1 class="text-2xl font-bold text-white mb-6">SENSOR LDR</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <td class="px-4 py-2 text-left text-sm font-medium text-gray-700">DATO</td>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($ft as $f)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $f->id }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $f->fot_intens_luz }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $f->fot_fecha }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $f->configuration->con_equipo }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="mt-6">
            {{ $ft->appends(request()->query())->links() }}
        </div>
    </div>
@endsection
