...
@section('content')
    <div class="max-w-7xl mx-auto 2 sm:px-6 lg:px-4">
        <h1 class="text-2xl font-bold text-white mb-6">SENSOR GUVA</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <td class="px-4 py-2 text-left text-sm font-medium text-gray-700">UV</td>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($guva as $g)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $g->id }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $g->uv_data }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $g->uv_fecha }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800">{{ $g->configuration->con_equipo }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            <div class="mt-6">
                {{ $guva->appends(request()->query())->links() }}
            </div>
        </div>
    @endsection
